...
public class AddTemporaryStationWalksGraphBuilder extends CreateNodesAndRelationships implements StationsWithDiversionRepository {
    ...
    private void recordDiversionsAtNode(final ImmutableGraphNode node, final GraphTransactionNeo4J txn) {
        ...
        final Stream<ImmutableGraphRelationshipNeo4J> outgoingDiversion = node.getRelationships(txn, GraphDirection.Outgoing, DIVERSION);

        final Set<DateTimeRange> ranges = outgoingDiversion.map(ImmutableGraphRelationshipNeo4J::getDateTimeRange).collect(Collectors.toSet());
