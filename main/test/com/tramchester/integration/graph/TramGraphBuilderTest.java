...
class TramGraphBuilderTest {
    ...
    void shouldHaveLinkRelationshipsCorrectForInterchange() {
        ...
        assertEquals(3, list.size(), "Wrong number of outbounds " + list);
    ...
    void shouldHaveLinkRelationshipsCorrectForNonInterchange() {
        ...
        Stream<GraphRelationship> outboundLinks = node.getRelationships(txn, Outgoing, LINKED);
    ...
    void shouldHaveCorrectOutboundsAtMediaCity() {
        ...
        Route tramRoutePiccEccles = tramRouteHelper.getYellow(when);
    ...
    void shouldHaveOutboundServiceForEveryInboundTrip() {
        ...
        transportData.getStations().forEach(station -> {
            ...
            callingRoutes.forEach(route -> {
                ...
                IdSet<Trip> allOutgoingTripsNotStartingHere = routeStationNode.getRelationships(txn, Outgoing, TO_SERVICE).
...
//                    toList();
            ...
            List<GraphRelationship> atHour = serviceNode.getRelationships(txn, Outgoing, TO_HOUR).
    ...
    void shouldHaveCorrectServiceRelationshipsAtRouteStationsAlongTrip() {
        ...
        callingTrips.forEach(trip -> {
            ...
            for (int seqNum = startSeqNum; seqNum <= endSeqNum; seqNum++) {
                ...
                List<GraphRelationship> toServices = routeStationNode.getRelationships(txn, Outgoing, TO_SERVICE).toList();
...
//        assertEquals(-1, minuteNodeIds.size());
        ...
        List<GraphRelationship> outboundToSvc = node.getRelationships(txn, Outgoing, TO_SERVICE).toList();
    ...
    void shouldHaveAllTimeNodesWithLinkToRouteStation() {
        ...
        long missing = timeNodes.filter(timeNode -> !timeNode.hasRelationship(txn, Outgoing, TRAM_GOES_TO)).count();
    ...
    void shouldHaveSameOutboundTripIdsForNeighbouringRouteStationWhenSameRouteAndSvc() {
        ...
        RouteStation routeStationA = getRouteStationWithCheck(stationA, buryToAlty);
        RouteStation routeStationB = getRouteStationWithCheck(stationB, buryToAlty);
        ...
        Set<GraphRelationship> svcOutboundsB = new HashSet<>(getRouteStationRelationships(routeStationB, Outgoing, relationshipTypes));
        ...
        assertFalse(stationAServicesForDate.isEmpty(), "no services at A for " + when + " from " + stationAServices);
...
//            assertTrue(fromA || fromB, serviceRepository.getServiceById(svcId).toString());
    ...
    void shouldHaveCorrectRelationshipsAtCornbrook() {
        ...
        RouteStation routeStationCornbrookAltyPiccRoute = getRouteStationWithCheck(cornbrook, tramRouteAltBury);
        List<GraphRelationship> outboundsA = getRouteStationRelationships(routeStationCornbrookAltyPiccRoute,
                Outgoing, transportRelationshipTypes);
...
//        assertTrue(fileSvcIdFromTrips.containsAll(serviceRelatIds));
    ...
    private @NonNull RouteStation getRouteStationWithCheck(Station station, Route route) {
        assertTrue(route.isAvailableOn(when), route.getId() + "Not available on " + when);
    ...
    private void checkInboundConsistency(Station station, Route route) {
        ...
        assertNotNull(routeStation,
                "On" + when + " could not find a route station for " + station.getId() + " and  " + route.getId());

        List<GraphRelationship> inbounds = getRouteStationRelationships(routeStation, GraphDirection.Incoming, transportRelationshipTypes);
