...
class RouteCostCalculatorTest {
    ...
    void shouldComputeCostsForMediaCityAshton() throws InvalidDurationException {
        assertEquals(Duration.ofMinutes(66), routeCostCalculator.getAverageCostBetween(txn, mediaCity, Ashton.from(stationRepository), when, modes));
        assertMinutesEquals(58, routeCostCalculator.getAverageCostBetween(txn,  Ashton.from(stationRepository), mediaCity, when, modes));
    }
    ...
    void shouldComputeSimpleCostBetweenStationsAltyBury() throws InvalidDurationException {
        ...
        assertEquals(Duration.ofMinutes(62).plusSeconds(3), buryToAlty);
        assertEquals(Duration.ofMinutes(62).plusSeconds(41), altyToBury);
    }
