...
class LiveDataHTTPFetcherTest {
    ...
    void shouldHaveTFGMKeyInConfig() {
        assertNotNull(configuration.getTfgmTramliveData().getDataSubscriptionKey(), "missing tfgm live data key");
    }
    ...
    void shouldHaveCrosscheckOnLiveDateDestinations() {
        ...
        ImmutableIdSet<Station> destinations = departureInfos.stream().flatMap(entry -> entry.getDueTrams().stream()).
        ...
        ImmutableIdSet<Station> mismatch = destinations.stream().filter(destinationId -> !expected.contains(destinationId)).
